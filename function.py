...
def choose_data_type():
    ...
    flag = True
    while flag:
        print('Выберите какие данные добавить: ', '1 - mobilephone', '2 - workphone', '3 - email', '4 - birthday', sep='\n')
        type_data = input('Выберите цифру (тип телефона): ')
        match type_data:
            case '1':
...
def create_new_contant():  # 6 - создать новый контакт
    ...
    if f_name in pb_local:
        return print("\033[31m {} \033[0m" .format('\nТакой контакт существует. Выберите пункт 9 в меню - добавить контактые данные\n'))
...
def edit_contact():  # 8 - редактировать телефонную книгу
    ...
    if answer != None:
        ...
        flag = True
        while flag:
            type_data = int(input('Выберите цифру (0, 1, 2 ...). Данные, которые хотите изменить: '))
...
def add_data():  # 9 - добавить контактные данные
    pb_local = load()
    print_all_fullname()
